...
public class DriverExtension implements WrapsDriver {
    ...
    protected void init() {
        ...
        if ((properties.getDefaultHub() != null) && (!properties.getDefaultHub().isEmpty())) {
            WebDriverFactory.setMode(WebDriverFactoryMode.THREADLOCAL_SINGLETON);
            WebDriverFactory.setDefaultHub(properties.getDefaultHub());
    ...
    public WebDriver getWrappedDriver() {
        return DriverTypeFactory.createDriver(properties.getBrowserType());
    }

    protected void stop() {
        DriverTypeFactory.closeBrowser(getWrappedDriver());
    }
    ...
    public void jsScrollIntoView(WebElement element) {
        try {
            CommonDriverUtils.waitForJQueryProcessing(getWrappedDriver(), properties.getTimeoutPageLoad());
    ...
    public <T extends WebElement> T isExistsReturnWebElement(T element) {
        ...
        getWrappedDriver().manage().timeouts().implicitlyWait(properties.getTimeoutImplicitlyWait(), TimeUnit.SECONDS);
    ...
    public <T> void checkUntil(T actual, final Matcher<? super T> matcher) {
        checkUntil(actual, matcher, TimeUnit.SECONDS.toMillis(properties.getTimeoutPageLoad()));
    }
}
